...
def prepare_dataset(samples_dir: Path, out_dir: Path, lang: str, quality: str, no_convert: bool = False) -> None:
    samples_dir = samples_dir.resolve()
    out_dir = out_dir.resolve()
    # remember if the output directory already existed before we start writing
    existed_before = "yes" if out_dir.exists() else "no"
...
""")
    ...
    config_path = out_dir / "voice_config.json"
    with config_path.open("w", encoding="utf-8") as f:
        json.dump(voice_config, f, indent=2)
    print("Wrote minimal voice config to:", config_path)
    # create a backup copy only if the directory existed already (meaning
    # the user might re-run init or training which could later overwrite)
    if existed_before == "yes" or (out_dir / "voice_config.dataset.json").exists():
        backup_path = out_dir / "voice_config.dataset.json"
        with backup_path.open("w", encoding="utf-8") as f:
            json.dump(voice_config, f, indent=2)
        print("Saved dataset config backup to:", backup_path)
...
def main():
    ...
    if args.cmd in ("init", "prepare"):
        try:
            prepare_dataset(args.samples_dir, args.out_dir, args.lang, args.quality, no_convert=args.no_convert)
